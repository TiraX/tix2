...
namespace tix
{
	enum EKEY_CODE
	{
		...
		KEY_SEMICOLON        = 0xBA,  // Semicolon  (;)	//US keyboard
	...
	enum DEVICE_EVENT_FLAG
	{
		DE_ALREAD_SENT      = 1 << 0,
        DE_IS_MULTI_INPUT   = 1 << 1,
		DE_HANDLED			= 1 << 2,
	};
	...
	struct TEvent
	{
		...
		TEvent& operator = (const _DeviceEvent& de)
		{
			type		= de.type;
			touch_id	= de._id;
			param		= de.param;
		...
		void	SetFlag(uint32 flag, bool enable)
		{
			if (enable)
				Flag		|= flag;
			else
	...
	{
	public:
        static const float heavy_force_value;
		...
		_DeviceEvent	Start;
		_DeviceEvent	End;
		_DeviceEvent	Current;
        float           MaxForce;
	};

	class TInput; 

	class TI_API TEventHandler
	{
	public:
		TEventHandler();
		...
		// !
		// return false to stop event continue pass to next
		//	--//
		virtual bool	OnEvent(const TEvent& e) = 0;
		virtual void	OnRegisterToInput(TInput * input);
