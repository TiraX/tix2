...
#	define DX_SETNAME(Resource, Name)
	...
	void FRHIDx12::CreateWindowsSizeDependentResources()
	{
		...
		// Create a root signature with a single constant buffer slot.
		{
			...
			RootSignature.GetParameter(1).InitAsDescriptorRange(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 3, D3D12_SHADER_VISIBILITY_PIXEL);
	...
	void FRHIDx12::SetShaderTexture(int32 BindIndex, FTexturePtr InTexture)
	{
		...
		D3D12_GPU_DESCRIPTOR_HANDLE Descriptor = DescriptorHeaps[EHT_CBV_SRV_UAV].GetGpuDescriptorHandle(TexDx12->TexDescriptor);
		CommandList->SetGraphicsRootDescriptorTable(BindIndex, Descriptor);
