...
#else	// VT_ENABLED
cbuffer MI_Parameter : register(b1)
{
	float4 CustomScalar0;
};
Texture2D<float4> MI_TexBaseColor : register(t0);
SamplerState LinearSampler : register(s0);

float4 GetBaseColor(in float2 texcoord)
{
	return MI_TexBaseColor.Sample(LinearSampler, texcoord) * (1.0 + CustomScalar0.x * 0.1);
}
#endif