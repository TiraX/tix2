...
FFluidSolverPbfGPU::FFluidSolverPbfGPU()
{
...
void FFluidSolverPbfGPU::UpdateParamBuffers(FRHI* RHI)
{
	if ((Flag & DirtyParams) != 0)
	{
		...
		UB_PbfParams->UniformBufferData[0].P0 = FFloat4(Particles.GetParticleMass(), Epsilon, Particles.GetParticleMass() / RestDenstiy, TimeStep / SubStep);
		const float h = Particles.GetParticleSeperation();
		// Calc cell size
		UB_PbfParams->UniformBufferData[0].P1 = FFloat4(h, h * h, 1.f / (h * h * h), 1.f / CellSize);
		UB_PbfParams->UniformBufferData[0].Dim = FInt4(Dim.X, Dim.Y, Dim.Z, Particles.GetTotalParticles());
...
void FFluidSolverPbfGPU::UpdateResourceBuffers(FRHI * RHI)
{
	...
	if ((Flag & DirtyParticles) != 0)
	{
		...
		RHI->UpdateHardwareResourceUB(UB_ParticlePositions, Particles.GetParticlePositions().data());
	}
...
void FFluidSolverPbfGPU::UpdateComputeParams(FRHI* RHI)
{
...
void FFluidSolverPbfGPU::Sim(FRHI * RHI, float Dt)
{
...
//void FFluidSolverPbfGPU::RenderParticles(FRHI* RHI, FScene* Scene, FMeshBufferPtr Mesh, FPipelinePtr Pipeline)
//{
