...
RWStructuredBuffer<uint4> CollectedClusters : register(u1); // x = Cluster Begin Index; y = Indirect Draw Command Index; z = Cluster Offset in this command
...
void main(uint3 groupId : SV_GroupID, uint3 threadIDInGroup : SV_GroupThreadID, uint3 dispatchThreadId : SV_DispatchThreadID)
{
	...
	if (InstanceMetaInfo[DrawCmdIndex].Info.y > 0)	// Test loaded primitives
	{
			...
			IntersectPlaneBBox(Planes[5], MinEdge, MaxEdge))
		{
            ...
            {
                CollectedClusters[CurrentClusterIndex + i].x = ClusterBegin;
                CollectedClusters[CurrentClusterIndex + i].y = DrawCmdIndex;
                CollectedClusters[CurrentClusterIndex + i].z = i;
                CollectedClusters[CurrentClusterIndex + i].w = 0;
            }
        }
	}
}
